...
class HomeController {
  async index(_req: any, res: any) {
    try {
      ...
      const productsGroupedByCategory = results
        .map((products, index) => ({
  ...
  async search(req: any, res: any) {
    ...
    try {
      ...
      const products = (await db.query(sql, [`%${name}%`, `%${name}%`, 'Active'])) as any[];
  ...
  async searchPreview(req: any, res: any) {
