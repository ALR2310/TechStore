...
class UserController {
  async index(req: any, res: any) {
    ...
    try {
      ...
      if (q) {
        ordersSql += ` AND Code LIKE "%${searchKey}%"`;
      }

      if (ordtype) {
        const statusMap = { '2': 'Processing', '3': 'Delivering', '4': 'Completed', '5': 'Cancelled' };
        if (statusMap[ordtype]) {
          ordersSql += ` AND Status = ?`;
          ordersParams.push(statusMap[ordtype]);
        }
      }
      ...
      const orderAddressQueries = orders.map((order) => ({
        sql: `SELECT * FROM Address WHERE Id = ? AND Status = ?`,
        params: [order.AdrId, 'Active'],
      }));
  ...
  async addressCreate(req: any, res: any) {
    ...
    try {
      ...
      const result: any = await db.query(sql, [req.user.Id, 1, 'Active']);
  ...
  async orderCreate(req: any, res: any) {
    ...
    try {
      const result: any = await db.query(`INSERT INTO Orders(Code, UserId, AdrId, TotalPrice) VALUES (?, ?, ?, ?)`, [
        orderCode,
        req.user.Id,
        addressId,
        totalPrice,
