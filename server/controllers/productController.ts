...
class ProductController {
  async index(req: any, res: any) {
    ...
    switch (price) {
      case '15':
        priceParams = 'p.Price <= 15000000';
        break;
      case '15-20':
        priceParams = 'p.Price BETWEEN 15000000 AND 20000000';
        break;
      case '20-25':
        priceParams = 'p.Price BETWEEN 20000000 AND 25000000';
        break;
      case '25-30':
        priceParams = 'p.Price BETWEEN 25000000 AND 30000000';
        break;
      case '30':
        priceParams = 'p.Price >= 30000000';
        break;
    }
    switch (cpu) {
      case 'intel-i9':
        cpuParams = "pd.DeviceCfg LIKE '%i9%' AND pd.DeviceCfg LIKE '%Intel%Core%'";
        break;
      case 'intel-i7':
        cpuParams = "pd.DeviceCfg LIKE '%i7%' AND pd.DeviceCfg LIKE '%Intel%Core%'";
        break;
      case 'intel-i5':
        cpuParams = "pd.DeviceCfg LIKE '%i5%' AND pd.DeviceCfg LIKE '%Intel%Core%'";
        break;
      case 'intel-i3':
        cpuParams = "pd.DeviceCfg LIKE '%i3%' AND pd.DeviceCfg LIKE '%Intel%Core%'";
        break;
      case 'amd-9':
        cpuParams = "pd.DeviceCfg LIKE '%Ryzen%' AND pd.DeviceCfg LIKE '%7%'";
        break;
      case 'amd-7':
        cpuParams = "pd.DeviceCfg LIKE '%Ryzen%' AND pd.DeviceCfg LIKE '%7%'";
        break;
      case 'amd-5':
        cpuParams = "pd.DeviceCfg LIKE '%Ryzen%' AND pd.DeviceCfg LIKE '%5%'";
        break;
    }
    switch (ram) {
      case '64':
        ramParams = "pd.DeviceCfg LIKE '%RAM; 64GB%'";
        break;
      case '32':
        ramParams = "pd.DeviceCfg LIKE '%RAM; 32GB%'";
        break;
      case '16':
        ramParams = "pd.DeviceCfg LIKE '%RAM; 16GB%'";
        break;
      case '8':
        ramParams = "pd.DeviceCfg LIKE '%RAM; 8GB%'";
        break;
      case '4':
        ramParams = "pd.DeviceCfg LIKE '%RAM; 4GB%'";
        break;
    }
    switch (vga) {
      case 'nvidia':
        vgaParams = "pd.DeviceCfg LIKE '%GeForce%' AND pd.DeviceCfg LIKE '%NVIDIA%'";
        break;
      case 'amd':
        vgaParams = "pd.DeviceCfg LIKE '%Radeon%' AND pd.DeviceCfg LIKE '%AMD%'";
        break;
    }
    switch (demand) {
      case 'office':
        demandParams = "p.ProdName LIKE '%Laptop%' AND p.ProdName NOT LIKE '%gaming%'";
        break;
      case 'gaming':
        demandParams = "p.ProdName LIKE '%Laptop%' AND p.ProdName LIKE '%gaming%'";
        break;
    }
    if (brand) {
    ...
    try {
      const category: any = await db.query('SELECT Id FROM Categories WHERE Slugs = ?', [slugs]);
      ...
      if (category?.length > 0) {
        ...
        const product: any = await db.query(sql, [slugs, 'Active']);
        ...
        if (product?.length > 0) {
          ...
          if (!viewedProducts.includes(product[0].Id)) {
            ...
            if (req.user) {
              const checkResult: any = await db.query(
                `SELECT COUNT(*) as count FROM ProductViewed WHERE ProdId = ? AND UserId = ?`,
          ...
          if (productRating[0].Total > 0) {
            ...
            productRating[0].Percentage5 =
              productRating[0].Percentage4 =
              productRating[0].Percentage3 =
              productRating[0].Percentage2 =
              productRating[0].Percentage1 =
                0;
          }
