...
BalloonEditor.create(document.querySelector("#ckeditor_balloon"))
  .then((editor) => {
...
function autoSelectCateOrBrand() {
  ...
  $("#product-brand-series option").each(function () {
    ...
    if (
      productName.includes(seriesText) &&
      seriesBrandId === parseInt(selectedBrandId)
    ) {
...
$("#product-brand").on("change", function () {
  ...
  if (selectedBrandId === "0")
    ...
    $("#product-brand-series option").each(function () {
      if (
        $(this).val() === "0" ||
        $(this).data("brand") === parseInt(selectedBrandId)
      )
        $(this).removeClass("d-none");
      else $(this).addClass("d-none");
    });
...
async function createBlobFromSource(url = "", fileInput = null) {
  if (fileInput && fileInput.files && fileInput.files.length > 0) {
    ...
    console.log("Không có file mới hoặc URL hợp lệ để tạo Blob.");
    return null;
  }
}
